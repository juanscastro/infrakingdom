...
NextJs es un proyecto open source construido y mantenido por Vercel, con lo resulta obvio que la primera opción es hostearlo con el servicio que proveen ellos, que es administrado, tiene un costo y es muy fácil de configurar.

Pero como somos animales del infrakingodm esta solución no es para nosotros. En cambio, vamos a ir con la opción de 'Self Hosting' es decir que vamos a provisionar nuestro servidor en una nube, en este caso Digital Ocean. Y vamos a hacerlo con una porción mucho menor del costo que tendriamos en Vercel y en el camino vamos a aprender sobre algunas herramientas muy utiles para el manejo de web servers.
...
4.  Además, vamos a utilizar los siguientes paquetes/aplicaciones: GIT, NodeJs, NPM, Nginx, Yarn, PM2.
5.  Configurar los DNS de tu dominio para que apunten a tu servidor.  

## Elije un Template y clona el repositorio

En nuestro caso hemos seleccionado Contentlayer Blog Starter, pero puedes encontrar algunos [aquí](https://vercel.com/templates).
...
Para el siguiente paso en mi caso he utilizado [GoDaddy](https://www.godaddy.com/), un proveedor bastante conocido y que ofrece distintios precios. En nuestro caso, registramos el dominio infrakingdom.xyz. Registrar un dominio no es una tarea compleja, te dejo [aquí](https://ar.godaddy.com/help) la página de soporte, además tienen habilitadas lineas telefónicas de asistencia al usuario.
...
Luego vamos a cambiar el nombre de la instancia o Hostname: en nuestro caso infrakingdom-blog. Tambien agregamos el Tag 'Blog'. Y por ultimo damos a 'Create Droplet'. En la proxima pantalla vamos a ver a nuestra instancia creandose.

Parte de las configuraciones adicionales que debemos hacer son reservar una ip estatica, resetear la password de root y configurar una regla de firewall para que nuestra instancia pueda ser accedida desde internet a traves del puerto 80.
...
yarn global add pm2
```bash
Ahora vamos a crear un directori para nuestro proyecto, en /var/www/ que es el estandard. 
```bash
...
```bash
Bien, si damos ls ahora podremos ver los archivos en nuestro directorio.
...
server {
    listen 80;
server_name nuestroproyecto.com nuestroproyecto.com;
...
```bash
Cerramos el editor con CTRL+X y confirmamos para guardar los cambios.
...
Este es el fin del tutorial, espero que les haya funcionado, en caso de que no, o por cualquier otro comentario pueden escribirme a traves de [LinkedIn](https://www.linkedin.com/in/juansimoncastro/)
